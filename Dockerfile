...
RUN apt-get update && apt-get install -y zip unzip git libpng-dev libpq-dev libzip-dev libaio-dev libc-client-dev libkrb5-dev ${PHPIZE_DEPS} && rm -r /var/lib/apt/lists/* &&   unzip /tmp/instantclient-basic-linux.x64-19.5.0.0.0dbru.zip -d /usr/local/ && unzip /tmp/instantclient-sdk-linux.x64-19.5.0.0.0dbru.zip -d /usr/local/ && ln -s /usr/local/instantclient_19_5 /usr/local/instantclient && pecl install -o -f redis && rm -rf /tmp/pear && docker-php-ext-configure pgsql -with-pgsql=/usr/local/pgsql && docker-php-ext-configure oci8 --with-oci8=instantclient,/usr/local/instantclient/ && docker-php-ext-configure imap --with-kerberos --with-imap-ssl &&  ln -s /usr/lib/libnsl.so.2.0.0  /usr/lib/libnsl.so.1 && docker-php-ext-install pdo pdo_pgsql pgsql oci8 imap gd zip && docker-php-ext-install gd zip && docker-php-ext-enable redis && rm -rf /tmp/* /var/lib/apt/lists/* && apt-get remove -y zip ${PHPIZE_DEPS} && docker-php-source delete
...
RUN sed -i 's/memory_limit = 128M/memory_limit = 500M/g' "$PHP_INI_DIR/php.ini"
