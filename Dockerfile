...
RUN apt-get clean && \ 
 apt-get update && \ 
 apt-get install -y libxml2-dev zip unzip git libpng-dev libpq-dev libzip-dev libaio-dev libc-client-dev libkrb5-dev ${PHPIZE_DEPS} && \ 
 ...
 docker-php-ext-install xml soap pdo pdo_pgsql pgsql oci8 imap gd zip && \ 
 docker-php-ext-install gd zip && \ 
 docker-php-ext-enable redis && \ 
 rm -rf /tmp/* /var/lib/apt/lists/* && \ 
 apt-get remove -y zip ${PHPIZE_DEPS} && \ 
